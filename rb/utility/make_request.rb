...
module ArcgisHubWorldCountriesGeneralizedUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ArcgisHubWorldCountriesGeneralizedResponse.new({})
    ...
    if fetch_err
      ...
      response = ArcgisHubWorldCountriesGeneralizedResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ArcgisHubWorldCountriesGeneralizedResponse.new(fetched)
    else
