...
class FeatureDirectTest < Minitest::Test
  def test_direct_list_feature
    setup = feature_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-feature", setup[:live] ? "live" : "unit")
    if _should_skip
...
def feature_direct_setup(mockres)
  ...
  if live
    ...
    client = ArcgisHubWorldCountriesGeneralizedSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
