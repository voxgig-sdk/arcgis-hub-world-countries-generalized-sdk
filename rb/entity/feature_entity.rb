...
class FeatureEntity
  ...
  def data_set(args)
    if args
      @_data = ArcgisHubWorldCountriesGeneralizedHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ArcgisHubWorldCountriesGeneralizedHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [FeatureListMatch, Hash, nil] match filter (any subset of
  #   Feature fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Feature>, Array] the matching Feature items; raises ArcgisHubWorldCountriesGeneralizedError on failure
