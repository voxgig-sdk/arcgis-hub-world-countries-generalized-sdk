...
from projectname_sdk import ArcgisHubWorldCountriesGeneralizedSDK
from core.spec import ArcgisHubWorldCountriesGeneralizedSpec
from core.result import ArcgisHubWorldCountriesGeneralizedResult
from core.response import ArcgisHubWorldCountriesGeneralizedResponse
...
def _make_ctx_from_map(ctxmap, client, utility):
    ...
    if isinstance(spec_map, dict):
        ctx.spec = ArcgisHubWorldCountriesGeneralizedSpec(spec_map)

    # Handle result from JSON map
    res_map = ctxmap.get("result")
    if isinstance(res_map, dict):
        ctx.result = ArcgisHubWorldCountriesGeneralizedResult(res_map)
        err_map = res_map.get("err")
        if isinstance(err_map, dict):
            msg = err_map.get("message", "")
            ctx.result.err = ArcgisHubWorldCountriesGeneralizedError("", msg)

    # Handle response from JSON map
    resp_map = ctxmap.get("response")
    if isinstance(resp_map, dict):
        ctx.response = ArcgisHubWorldCountriesGeneralizedResponse(resp_map)
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return ArcgisHubWorldCountriesGeneralizedError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except ArcgisHubWorldCountriesGeneralizedError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = ArcgisHubWorldCountriesGeneralizedResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_add_basic(self):
        ...
        feature = ArcgisHubWorldCountriesGeneralizedBaseFeature()
        utility.feature_add(ctx, feature)

        assert len(client.features) == start_len + 1

    def test_feature_hook_basic(self):
        hook_client = ArcgisHubWorldCountriesGeneralizedSDK.test(None, None)
        hook_utility = hook_client._utility
        ctx = _make_test_ctx(hook_client, hook_utility)

        state = {"called": False}

        class TestHookFeature(ArcgisHubWorldCountriesGeneralizedBaseFeature):
    ...
    def test_feature_init_basic(self):
        init_client = ArcgisHubWorldCountriesGeneralizedSDK.test(None, None)
        ...
        class TestInitFeature(ArcgisHubWorldCountriesGeneralizedBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(ArcgisHubWorldCountriesGeneralizedBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = ArcgisHubWorldCountriesGeneralizedSDK({
            "system": {
                "fetch": mock_fetch,
            },
        })
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = ArcgisHubWorldCountriesGeneralizedSDK({
            "system": {
                "fetch": mock_fetch,
            },
        })
    ...
    def test_make_fetch_def_basic(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = ArcgisHubWorldCountriesGeneralizedSpec({
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = ArcgisHubWorldCountriesGeneralizedSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = ArcgisHubWorldCountriesGeneralizedSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = ArcgisHubWorldCountriesGeneralizedOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = ArcgisHubWorldCountriesGeneralizedSDK.test(None, setup_opts)
