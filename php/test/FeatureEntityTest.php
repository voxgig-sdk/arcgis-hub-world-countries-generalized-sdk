...
class FeatureEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = ArcgisHubWorldCountriesGeneralizedSDK::test($seed, null);
        $seen = iterator_to_array($base->Feature(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = ArcgisHubWorldCountriesGeneralizedConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = ArcgisHubWorldCountriesGeneralizedSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Feature(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_FEATURE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function feature_basic_setup($extra)
{
    ...
    $client = ArcgisHubWorldCountriesGeneralizedSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_FEATURE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_FEATURE_ENTID"]);
    ...
    if ($env["ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_LIVE"] === "TRUE") {
        ...
        $client = new ArcgisHubWorldCountriesGeneralizedSDK(Helpers::to_map($merged_opts));
    }
