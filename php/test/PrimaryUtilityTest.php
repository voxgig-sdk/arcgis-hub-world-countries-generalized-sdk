...
// ArcgisHubWorldCountriesGeneralized SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        ArcgisHubWorldCountriesGeneralizedSDK $client,
        ArcgisHubWorldCountriesGeneralizedUtility $utility
    ): ArcgisHubWorldCountriesGeneralizedContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new ArcgisHubWorldCountriesGeneralizedContext($ctxmap, null);
        ...
        if (isset($ctxmap['spec']) && is_array($ctxmap['spec'])) {
            $ctx->spec = new ArcgisHubWorldCountriesGeneralizedSpec($ctxmap['spec']);
        }

        // Handle result from JSON map
        if (isset($ctxmap['result']) && is_array($ctxmap['result'])) {
            $ctx->result = new ArcgisHubWorldCountriesGeneralizedResult($ctxmap['result']);
            if (isset($ctxmap['result']['err']) && is_array($ctxmap['result']['err'])) {
                $msg = $ctxmap['result']['err']['message'] ?? '';
                $ctx->result->err = new ArcgisHubWorldCountriesGeneralizedError('', $msg);
            }
        }

        // Handle response from JSON map
        if (isset($ctxmap['response']) && is_array($ctxmap['response'])) {
            $ctx->response = new ArcgisHubWorldCountriesGeneralizedResponse($ctxmap['response']);
    ...
    private static function fixctx(ArcgisHubWorldCountriesGeneralizedContext $ctx, ArcgisHubWorldCountriesGeneralizedSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?ArcgisHubWorldCountriesGeneralizedError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ArcgisHubWorldCountriesGeneralizedError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new ArcgisHubWorldCountriesGeneralizedResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new ArcgisHubWorldCountriesGeneralizedBaseFeature();
    ...
    {
        $hook_client = ArcgisHubWorldCountriesGeneralizedSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends ArcgisHubWorldCountriesGeneralizedBaseFeature {
            public $hook_fn = null;
            public function TestHook(ArcgisHubWorldCountriesGeneralizedContext $ctx): void
            {
    ...
    {
        $init_client = ArcgisHubWorldCountriesGeneralizedSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends ArcgisHubWorldCountriesGeneralizedBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new ArcgisHubWorldCountriesGeneralizedSDK([
            'system' => [
    ...
    {
        $blocked_client = new ArcgisHubWorldCountriesGeneralizedSDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new ArcgisHubWorldCountriesGeneralizedSpec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new ArcgisHubWorldCountriesGeneralizedSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = ArcgisHubWorldCountriesGeneralizedSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new ArcgisHubWorldCountriesGeneralizedOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = ArcgisHubWorldCountriesGeneralizedSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof ArcgisHubWorldCountriesGeneralizedError) ? $result->err->msg : (string)$result->err;
