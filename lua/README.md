...
The Lua SDK for the ArcgisHubWorldCountriesGeneralized API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Feature(nil):list(nil, nil)
...
local result, err = client:ArcgisHubWorldCountriesGeneralized(nil):load(
  { id = "test01" }, nil
)
