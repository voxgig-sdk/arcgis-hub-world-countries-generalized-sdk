# ArcgisHubWorldCountriesGeneralized Lua SDK



The Lua SDK for the ArcgisHubWorldCountriesGeneralized API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Feature()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local feature, err = client:Feature():load()
    if err then error(err) end
...
```
lua/
├── arcgis-hub-world-countries-generalized_sdk.lua    -- Main SDK module
...
The main module (`arcgis-hub-world-countries-generalized_sdk`) exports the SDK constructor
