...
AQL-driven command-line client **and** interactive REPL for the ArcgisHubWorldCountriesGeneralized
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/arcgis-hub-world-countries-generalized-cli)
make build

# 2. See usage (words, entities, env vars)
./arcgis-hub-world-countries-generalized-cli --help
...
./arcgis-hub-world-countries-generalized-cli list metadata

# 5. Override the API base URL for a single call
ARCGIS_HUB_WORLD_COUNTRIES_GENERALIZED_BASE=https://api.example.com ./arcgis-hub-world-countries-generalized-cli list feature

# 6. No arguments -> interactive REPL
./arcgis-hub-world-countries-generalized-cli
arcgis-hub-world-countries-generalized> list feature
arcgis-hub-world-countries-generalized> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/arcgis-hub-world-countries-generalized-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/arcgis-hub-world-countries-generalized-cli list feature
   ```
...
Run with no arguments to open a REPL (prompt `arcgis-hub-world-countries-generalized>`). Each line is
evaluated as its own AQL expression:

```text
$ ./arcgis-hub-world-countries-generalized-cli
arcgis-hub-world-countries-generalized> list feature
arcgis-hub-world-countries-generalized> /help
arcgis-hub-world-countries-generalized> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `feature`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/arcgis-hub-world-countries-generalized-cli`. |
...
(`./arcgis-hub-world-countries-generalized-cli <expr>`) and interactively (the REPL), and expressions compose the
