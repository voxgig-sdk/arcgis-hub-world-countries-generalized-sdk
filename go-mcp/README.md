...
[MCP](https://modelcontextprotocol.io) server exposing the ArcgisHubWorldCountriesGeneralized SDK as
two agent tools — `arcgis-hub-world-countries-generalized_list` and `arcgis-hub-world-countries-generalized_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/arcgis-hub-world-countries-generalized-mcp)
make build

# 2. Provide credentials via the environment
export ARCGIS_HUB_WORLD_COUNTRIES_GENERALIZED_APIKEY=sk_live_xxx
...
```jsonc
// arcgis-hub-world-countries-generalized_list: first page of records
{ "entity": "feature" }
{ "entity": "feature", "query": { } }

// arcgis-hub-world-countries-generalized_load: one record by id
{ "entity": "feature", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/arcgis-hub-world-countries-generalized-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user arcgis-hub-world-countries-generalized \
     -- "$PWD"/dist/*/arcgis-hub-world-countries-generalized-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `arcgis-hub-world-countries-generalized_list` and `arcgis-hub-world-countries-generalized_load` tools now appear
   in new sessions. Ask the agent to *"list feature using arcgis-hub-world-countries-generalized"*
   and it calls `arcgis-hub-world-countries-generalized_list` with `{"entity":"feature"}`.
...
```sh
export ARCGIS_HUB_WORLD_COUNTRIES_GENERALIZED_APIKEY=sk_live_xxx            # API key
export ARCGIS_HUB_WORLD_COUNTRIES_GENERALIZED_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `arcgis-hub-world-countries-generalized_list` tool
...
### Call the `arcgis-hub-world-countries-generalized_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `arcgis-hub-world-countries-generalized_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `arcgis-hub-world-countries-generalized_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ARCGIS_HUB_WORLD_COUNTRIES_GENERALIZED_APIKEY` | API key sent with every request. |
| `ARCGIS_HUB_WORLD_COUNTRIES_GENERALIZED_BASE` | Optional override of the API base URL. |
