...
class TestFeatureEntity:
    ...
    def test_should_stream(self):
        ...
        base = ArcgisHubWorldCountriesGeneralizedSDK.test(seed, None)
        seen = list(base.Feature(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = ArcgisHubWorldCountriesGeneralizedSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Feature(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_FEATURE_ENTID JSON to run live")
        client = setup["client"]
...
def _feature_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_FEATURE_ENTID"))
    ...
    if env.get("ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_LIVE") == "TRUE":
        ...
        client = ArcgisHubWorldCountriesGeneralizedSDK(helpers.to_map(merged_opts))

    _live = env.get("ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("ARCGISHUBWORLDCOUNTRIESGENERALIZED_TEST_EXPLAIN") == "TRUE",
        "live": _live,
