...
The Python SDK for the ArcgisHubWorldCountriesGeneralized API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Feature(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.ArcgisHubWorldCountriesGeneralized(None).load(
    {"id": "test01"}, None
)
